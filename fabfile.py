...
_REMOTE_BASE_DIR = '/home/www/web/py3-web'
...
#     '''
#     dt = _now()
#     f = 'backup-py3_test-%s.sql' % dt
#     with cd('/tmp'):
#         run('mysqldump --user=%s --password=%s --skip-opt --add-drop-table --default-character-set=utf8 --quick py3-web > %s' % (db_user, db_password, f))
...
#     sqls = [
#         'drop database if exists py3-web;',
#         'create database py3-web;',
#         'grant select, insert, update, delete on py3-web.* to \'%s\'@\'localhost\' identified by \'%s\';' % (db_user, db_password)
#     ]
#     for sql in sqls:
#         local(r'mysql -uroot -p%s -e "%s"' % (p, sql))
#     with lcd(backup_dir):
#         local('tar zxvf %s' % restore_file)
#     local(r'mysql -uroot -p%s py3-web < backup/%s' % (p, restore_file[:-7]))
#     with lcd(backup_dir):
